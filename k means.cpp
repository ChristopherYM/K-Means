...
int main(int argc, char **argv){
	//Leer archivo 
	ifstream archivo("yellow_tripdata_2015-01-06.csv");
	if (!archivo.is_open()) std::cout << "ERROR: File Open" << '\n';
	int start, numpoint = 7291;
	string A, B, C, D, E, F, G, H, I, J, K, L, M, N, O, P, Q, R, S;

	sf::RenderWindow win(sf::VideoMode(Ancho, Largo, 32), "K-Means visualization");
	...
	start = 0;
	while (archivo.good() && start < numpoint){
		getline(archivo, A, ','); 
		getline(archivo, B, ','); 
		getline(archivo, C, ',');
		getline(archivo, D, ','); 
